...
Recall the work of RV.

\subsection{Full-Adjunctions}

How to phrase unicity of higher adjunctions?

\subsection{The rest}

Recall that a morphism $f\in A \to B \in \A$ in an $(\infty,n)$-category has a left/right adjoint if it has a left/right adjoint in the bicategory $\tau_{\leq 2}\A$. [Cite RV]

\begin{definition}[Has Adjoints] Let $\A$ be an $(\infty,n)$-category. We say that $\A$ \textbf{has adjoints} if all $k$-morphisms have a left and right adjoint for $1 \leq k \leq n-1$.
\end{definition}

\begin{definition}[Fully-Adjoint] By \textbf{induction} on $n$, we define the following. Let $\A$ be an $(\infty,n)$-category. A morphism $f\in A \to B \in \A$ is $\textbf{fully-adjoint}$ if:
\begin{enumerate}
...
\begin{conjecture}[almost Lurie Claim 2.3.19] The $(\infty,1)$-category $(\infty,n)\textnormal{-}\Cat^{\textnormal{adj}}$ of (small) $(\infty,n)$-categories with adjoints is a \textbf{coreflexive subcategory} of the $(\infty,1)$-category $(\infty,n)\textnormal{-}\Cat$ of (small) $(\infty,n)$-categories. We denote the coreflector by $\tau_{\textnormal{adj}}$.
\end{conjecture}
...
If the category $\A$ is monoïdal, one can define what it means for objects to be fuly-adjoints themselves:
