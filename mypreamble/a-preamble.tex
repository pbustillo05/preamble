...
\usepackage{pgfplots}
\usepackage{aliascnt}    %to have correct referencing of theorems in thmenv.sty 

%%%%% MY SPECIAL PACKAGES
\usepackage{../mypreamble/mathenv}     %improves on AMSmath's equation (path resolvable from main document)
\usepackage{../mypreamble/thmenv}     %replaces the AMSthm package (path resolvable from main document)

%%%%% MATH MACROS AND FONT PACKAGES
%\usepackage{fontspec}
\usepackage{../mypreamble/mathcro}
\usepackage{../mypreamble/catcro}
...
\def\conventionautorefname{Convention}
...
%%%%% QUIVER SETUP
\usetikzlibrary{calc,decorations.pathmorphing,spath3}
